...
- **Умный дом = код-модуль поверх внешнего Home Assistant.** Модуль `homeassistant` (`modules/homeassistant/`) не имеет своей БД: состояние живёт в Home Assistant, Domo проксирует `GET /api/states` и `POST /api/services/<domain>/<service>` через серверные роуты `GET/POST /api/modules/homeassistant/states|call`. Безопасность: `HA_URL`/`HA_TOKEN` читаются на сервере из `process.env` и никогда не уходят клиенту; разрешён только белый список сервисов (ALLOWED_SERVICES: light/switch/fan/climate/cover/media_player/humidifier); `entity_id` и домен валидируются (защита от path traversal/SSRF); вызовы — только с правом `can.write` модуля (`core.can`). UI группирует устройства по доменам, пуллит состояния раз в 15 с, управление скрыто без `canWrite`. Проверяется интеграционным тестом `apps/server/test/homeassistant.test.ts` (фейковый HA-сервер в процессе).
