...
- **Покупки для дома (RICE).** Список покупок с приоритизацией: идеи записываются в бэклог без оценок. Для попадания в список нужны четыре ручные оценки (охват, польза, уверенность, сложность по шкале 1–5) и примерная сумма — денежный коэффициент система выводит сама, логарифмической шкалой между самой дешёвой и самой дорогой покупкой списка; балл `(охват × польза × уверенность) ÷ ((деньги + сложность) ÷ 2)` ставит покупку на место. Статусы «хочу → в планах → куплено» с архивом.
...
Флаг `"publicRead": true` в манифесте открывает read-only доступ к сущностям модуля **без авторизации** (CRUD-запись и `GET /manifest` остаются за `core.can`). Это для страниц, которыми делятся с гостями без аккаунта — например, список желаний к празднику. Сервер валидирует только поля сущностей (не манифест, не их содержимое).

Пример — `modules/wishlist/` («Подарки»): это код-модуль (`kind: "code"`), гостевой экран отдаёт свой фронтенд по пути `/wishlist` (`apps/web/src/pages/Wishlist.tsx`). Кроме публичного чтения он умеет **бронирование подарков**: гость нажимает «Я подарю» и вводит логин и 6-значный пинкод любого существующего пользователя — бронь привязывается к нему и видна на публичной странице как «Подарит: логин»; снять бронь может только тот, кто её поставил. Администратор может **назначить гостя** (в управляемом карточке: «Назначить гостя» → ввести имя), тогда на публичной странице бейдж показывает «Подарит: Тётя Галя» и снять метку может только администратор (кнопка «Снять» в управляемом карточке). Публичные роуты `POST /gift/:id/book|unbook` аутентифицируются пинкодом на сервере через `core.users.verifyPin` (без создания сессии) и делят общий лимит частоты (30 попыток на IP за 15 минут, бронь + снятие вместе). CRUD и роуты `/assign|/release` — по обычной схеме через `/m/wishlist`.
...
Манифест с `kind: "code"` + Fastify-плагин (`server.ts`) и React-роуты (`ui.tsx`). Роуты регистрируются через `ctx.route({ method, path, action }, handler)` — проверка прав обязательна для каждого роута. Для публичных мутаций без входа роут помечается `public: true` и сам аутентифицирует запрос (логин+пинкод), при необходимости с per-route limit (`config.rateLimit`). Примеры — `modules/admin/`, `modules/wishlist/` (публичная страница с бронированием по пинкоду). Подробности: [docs/architecture.md](docs/architecture.md).
